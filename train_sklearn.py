...
def run_single_experiment(args, varID_0, varID_1, dataloader_0, dataloader_1,
    ...
    bins = np.linspace(0, 60, 61).tolist() + \
        np.linspace(62, 100, 11).tolist() + [110, 130, 160, 10000]
    model = Likelihood2D((8, 8))
    ...
    logger.info('ROC {} = {}'.format(varID_1, roc_auc_est))
...
if __name__ == '__main__':
   ...
   # for i, ((d_0, d_val_0, d_test_0), (d_1, d_val_1, d_test_1)) in enumerate(izip(dataloaders_0, dataloaders_1))
    ...
    logger.info('{} experiments finished. Results:'.format(
        len(experiment_runs)))
    ...
    for run in experiment_runs:
        ...
        logger.info(
            'Experiment {experiment_id}: ROC({variation}) = {ROC_AUC}'.format(**run))
